...
#include <ql/experimental/swaptions/discretizedirregularswap.hpp>

namespace QuantLib {

    DiscretizedIrregularSwap::DiscretizedIrregularSwap(const IrregularSwap::arguments& args,
    ...
    void DiscretizedIrregularSwap::reset(Size size) {
        values_ = Array(size, 0.0);
        adjustValues();
    }

    std::vector<Time> DiscretizedIrregularSwap::mandatoryTimes() const {
    ...
    void DiscretizedIrregularSwap::preAdjustValuesImpl() {
		// floating payments
        for (Size i=0; i<floatingResetTimes_.size(); i++) {
            Time t = floatingResetTimes_[i];
            if (t >= 0.0 && isOnTime(t)) {
                ...
                Real nominal = arguments_.floatingNominals[i];
                Time T = arguments_.floatingAccrualTimes[i];
                Spread spread = arguments_.floatingSpreads[i];
                Real accruedSpread = nominal*T*spread;

                //New Code:  
				//Real floatingCoupon = arguments_.floatingCoupons[i];

                for (Size j=0; j<values_.size(); j++) {

					
					//Old code assumes forward curve equals discounting curve
					  Real coupon = nominal * (1.0 - bond.values()[j])
                                + accruedSpread * bond.values()[j];
					

					//New Code: Enabling multicurve functionality
					//Real coupon = floatingCoupon*bond.values()[j];


                    if (arguments_.type == IrregularSwap::Payer)
                        values_[j] += coupon;
                    else
                        values_[j] -= coupon;
                }
    ...
    void DiscretizedIrregularSwap::postAdjustValuesImpl() {
