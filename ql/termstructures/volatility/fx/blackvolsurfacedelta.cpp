...
QuantLib::ext::shared_ptr<FxSmileSection> BlackVolatilitySurfaceDelta::blackVolSmile(Time t) const {
    ...
    for (Real delta : putDeltas_) {
        ...
        try {
            BlackDeltaCalculator bdc(Option::Put, dt, spot, dDiscount, fDiscount, vol * std::sqrt(t));
    ...
    if (hasAtm_) {
        Real vol = interpolators_.at(i)->blackVol(t, 1, true);
        try {
            BlackDeltaCalculator bdc(Option::Put, atmDt, spot, dDiscount, fDiscount, vol * std::sqrt(t));
    ...
    for (Real delta : callDeltas_) {
        Real vol = interpolators_.at(i)->blackVol(t, 1, true);
        try {
            BlackDeltaCalculator bdc(Option::Call, dt, spot, dDiscount, fDiscount, vol * std::sqrt(t));
            Real strike = bdc.strikeFromDelta(delta);
            if (smileSection.count(strike) == 0)
                smileSection[strike] = vol;
        } catch (const std::exception& e) {
